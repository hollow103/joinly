...
| Interfaz | Componentes propios de React Native + `StyleSheet` y una librería atómica interna (`src/ui/`) con tokens | Cumple la guía de `AGENTS.md` y el objetivo WCAG 2.1 AA sin dependencia pesada |
| Descubrimiento | Lista / rejilla con distancia y zona aproximada textual; **sin mapa** | Coincide con `docs/04-requisitos.md` ("lista o rejilla"); cero claves de API y cero cuenta de facturación. Un mapa se evalúa tras el piloto |
...
| Descubrir (lista/rejilla) | `POST /events/search` | Ubicación actual (permiso contextual) o zona manual; radio elegible; "ampliar radio" si vacío |
...
| **M2 · Descubrimiento** | Formulario de búsqueda (ubicación actual vs zona manual, radio), `POST /events/search`, lista/rejilla con distancia y zona aproximada, "ampliar radio", scroll infinito por cursor, ficha `GET /events/{id}` condicionada por visibilidad | Se listan eventos sembrados; sin resultados aparece la acción de ampliar radio; la ficha no muestra ubicación exacta sin participación |
