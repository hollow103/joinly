...
| Flujo central crear, descubrir y participar | ~80% | M2 cerrado con recorrido interactivo contra Render; M3 y M4 verificados por API, envío del formulario M3 con teclado pendiente de dispositivo real |
...
| **M2 · Descubrimiento** ✅ | Patrón Radar de planes de `docs/19`: formulario de búsqueda (ubicación actual, radio), radar abstracto, lista con distancia y zona aproximada, "ampliar radio", scroll infinito por cursor y ficha `GET /events/{id}` condicionada por visibilidad | **Cerrado (2026-09-02).** Recorrido interactivo en el simulador iOS contra el backend de Render: buscar con ubicación, radio, radar con blips, estado vacío → "Ampliar a 6 km", chips de tiempo ahora en servidor (`startsAfter`/`startsBefore` → "Esta tarde" 1 evento, "Este finde" 2), lista por cursor y ficha de tercero sin `exactLocation` (aviso "solo tras confirmar"), con "Bloquear"/"Reportar"/"Unirme". |
| **M3 · Crear y gestionar** 🟡 | `POST /events` con validación (categorías, futuro, capacidad, acceso), `GET /me/events` con filtro de estado, `PATCH` con `If-Match`, `POST /cancellation`, edición de `notes`; entrada desde el botón circular naranja del Radar y enlace "Mis planes" | Implementado y verificado por API contra Render (22/22: creación, `startsAt` pasado → `422`, categoría inválida → `422`, **cuarto activo → `409 active_event_limit_reached`**, `GET /me/events` por estado, `PATCH` diff con `If-Match` → `200`, obsoleto → `412`, sin `If-Match` → `428`, `notes`, `POST /cancellation` → `204`, evento empezado → solo notas). En la app: el formulario de tres bloques y "Mis planes" renderizan con datos reales; los campos de fecha/hora de texto libre se sustituyeron por un **selector nativo** (`ui/DateTimeField`, `@react-native-community/datetimepicker`), verificado en el simulador. **Pendiente:** el envío completo del formulario con teclado desde la interfaz (idb no enfoca los `TextInput` de RN en el simulador; se cierra en un dispositivo real o con el recorrido manual). |
...
1. **M2 · Descubrimiento.** Cerrado el 2026-09-02 (recorrido interactivo en simulador iOS contra Render).
...
8. RESUELTO (2026-09-02): crear/editar usan un selector de fecha/hora nativo
   (`ui/DateTimeField`), no campos de texto. El texto original decía:
   **Fecha/hora en crear/editar evento.** Son campos de texto. ¿M6 introduce
