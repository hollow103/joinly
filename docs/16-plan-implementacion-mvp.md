...
**Estado:** Fases 0 a 3 completadas. Fase 1 se cerró para el MVP controlado tras ejecutar la integración con un JWT real de Supabase y aprobar los textos v1 provisionales; los datos legales ficticios deben sustituirse antes de una apertura pública. Fase 4 implementa reportes, cola administrativa, decisiones auditadas, preferencias push, solicitud de eliminación, retención y la entrega de notificaciones push por Expo (registro transaccional más despacho programado, idempotente y con un solo intento por notificación); queda pendiente el panel de moderación. Fase 6 ya cierra eventos terminados de forma programada e idempotente; siguen pendientes límites, logging operativo y distribución. En Fase 5, el frontend movil ha completado y verificado M0 (andamiaje Expo) y M1 (identidad y perfil, registro y alta contra Supabase probados en el emulador el 2026-09-01); M2 (descubrimiento) esta implementado y pendiente del recorrido manual completo; M3 (crear y gestionar) y M4 (participar) estan implementados y verificados contra el backend real (M4 incluye union directa/aprobacion/privada, reintento idempotente sin duplicar, aprobar con `If-Match`, abandonar e invitaciones); M5 (bloqueos y ajustes) esta implementado y pendiente de aceptacion manual. El flujo movil de reportes permite reportar un evento o a su creador mediante `POST /reports`; está pendiente de aceptación manual. M6 no esta implementado. El panel de moderacion (`admin/`) tiene su primera iteracion: proyecto Vite + React + TypeScript + Tailwind + Tremor, login Supabase con comprobacion del rol `admin` contra `GET /me`, cola de reportes por estado con paginacion por cursor y decision (resolver/archivar con accion y nota, enviada con `If-Match`); el dashboard con la grafica de eventos creados y las estadisticas de uso queda para una segunda iteracion junto con un endpoint `GET /admin/metrics` nuevo. El registro de preferencias push y la entrega push por Expo estan implementados en backend; la app aun no obtiene un token real fuera de una build de desarrollo o standalone.
...
**Estado:** implementación backend parcial completada. Existen reportes, decisiones auditadas, ocultación/suspensión, preferencias push, solicitudes de eliminación, retención y la entrega de notificaciones push. El módulo `com.joinly.backend.notifications` registra la notificación dentro de la transacción del caso de uso (`NotificationRecorder`) y `NotificationDispatchService` la envía por Expo en un lote programado, con `FOR UPDATE SKIP LOCKED` para no reenviar, un solo intento por notificación, respeto de las preferencias por tipo y borrado del token ante `DeviceNotRegistered`. Configuración en `joinly.notifications.*` (cron, kill switch, tamaño de lote, endpoint y access token opcional de Expo). B-14 lo cubre de forma hermética. El panel `admin/` cubre ya login, control de rol `admin`, cola de reportes y decisiones con `If-Match`; queda pendiente el dashboard de métricas (endpoint `GET /admin/metrics` + gráficas) y la validación operativa de cierre de fase.
...
4. Crear el panel Vite mínimo con inicio de sesión, cola de reportes, detalle y acciones de moderación. **Primera iteración hecha** (`admin/`); pendiente el dashboard de métricas de uso con un endpoint `GET /admin/metrics` nuevo.
...
verificados contra el backend, M2 y M5 implementados). El reporte movil esta
implementado y pendiente de aceptación manual. Solo falta M6
...
**Estado:** `EventClosingService` ejecuta cada minuto el cierre persistente de eventos `published` cuyo final ya pasó. El `UPDATE` condicional incrementa la versión una vez y hace el proceso repetible e idempotente. La frecuencia se configura mediante `joinly.events.closing-cron`; en pruebas se desactiva el disparador y se invoca el servicio directamente. B-12 cubre el ciclo completo por API: descubrimiento previo, transición a `closed`, retirada para terceros, visibilidad del creador y repetición idempotente.
